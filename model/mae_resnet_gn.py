...
class BasicBlock(nn.Module):
    """Basic ResNet block for ResNet-18/34, with optional ConvNeXt-style main path.
       When convnext_style=True:
         main: DWConv7x7(stride) -> GN -> PW 1x1 (C -> 4C) -> GELU -> Drop -> GN -> PW 1x1 (4C -> C)
       Otherwise (default):
         main: Conv3x3(stride) -> GN -> ReLU -> Drop -> Conv3x3 -> GN
    """
    expansion = 1
    ...
    def __init__(
        ...
        convnext_style: bool = False,
        resnext_style: bool = False,
        no_relu: bool=False,
        resnet_bottleneck:int=0, # when > 0 : divide channels by resnet_bottleneck
    ):
        super().__init__()
        self.resnet_bottleneck = resnet_bottleneck
        self.convnext_style = bool(convnext_style)
        self.resnext_style = bool(resnext_style)
        self.downsample = downsample
        self.drop = nn.Dropout(dropout_prob) if dropout_prob > 0 else nn.Identity()

        if self.resnet_bottleneck > 0:
            middle_channels = out_channels // self.resnet_bottleneck
            self.convs = nn.Sequential(
                nn.Conv2d(in_channels, middle_channels, kernel_size=1, stride=1, bias=False),
                nn.GroupNorm(_choose_gn_groups(middle_channels, gn_max_groups), middle_channels),
                nn.ReLU(inplace=True),
                nn.Conv2d(middle_channels, middle_channels, kernel_size=3, padding=1, bias=False, stride=stride),
                nn.GroupNorm(_choose_gn_groups(middle_channels, gn_max_groups), middle_channels),
                nn.ReLU(inplace=True),
                nn.Conv2d(middle_channels, out_channels, kernel_size=1, bias=False),
            )

        if not self.convnext_style and not self.resnext_style:
            # --- Classic BasicBlock (your original layout) ---
            self.conv1 = nn.Conv2d(in_channels, out_channels, 3, stride=stride, padding=1, bias=False)
            self.bn1   = nn.GroupNorm(_choose_gn_groups(out_channels, gn_max_groups), out_channels)
            self.relu  = nn.ReLU(inplace=True) if not no_relu else nn.Identity()
            self.conv2 = nn.Conv2d(out_channels, out_channels, 3, padding=1, bias=False)
            self.bn2   = nn.GroupNorm(_choose_gn_groups(out_channels, gn_max_groups), out_channels)
        elif self.convnext_style:
            self.dw = nn.Conv2d(in_channels, in_channels, kernel_size=7, stride=stride, padding=3,
                                groups=in_channels, bias=False)
            self.dw_gn = nn.GroupNorm(_choose_gn_groups(in_channels, gn_max_groups), in_channels)
            hidden = 8 * out_channels 
            self.pw1 = nn.Conv2d(in_channels, hidden, kernel_size=1, bias=False)
            self.pw1_gn = nn.GroupNorm(_choose_gn_groups(hidden, gn_max_groups), hidden)
            self.act = nn.GELU() if not no_relu else nn.Identity()
            self.pw2 = nn.Conv2d(hidden, out_channels, kernel_size=1, bias=False)
            self.pw2_gn = nn.GroupNorm(_choose_gn_groups(out_channels, gn_max_groups), out_channels)

            # Keep a ReLU *after* the residual add for parity with BasicBlock API
            self.relu = nn.ReLU(inplace=True)
        else:
            self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=stride, bias=False)
            self.conv_nxt = nn.Conv2d(out_channels, out_channels, kernel_size=7, padding=3, bias=False, groups=16)
            self.bn_nxt = nn.GroupNorm(_choose_gn_groups(out_channels, gn_max_groups), out_channels)
            self.bn1   = nn.GroupNorm(_choose_gn_groups(out_channels, gn_max_groups), out_channels)
            self.relu  = nn.ReLU(inplace=True) if not no_relu else nn.Identity()
            self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=1, bias=False)
            self.bn2   = nn.GroupNorm(_choose_gn_groups(out_channels, gn_max_groups), out_channels)

        # If not provided, build a standard downsample when shape/stride mismatch.
    ...
    def forward(self, x, return_residual: bool = False):
        identity = x
        if self.resnet_bottleneck > 0:
            residual_branch = self.convs(x)
        elif (not self.convnext_style and not self.resnext_style):
            out = self.relu(self.bn1(self.conv1(x)))
            out = self.drop(out)
            residual_branch = self.bn2(self.conv2(out))
        elif self.convnext_style:
            out = self.dw_gn(self.dw(x))
            out = self.pw1_gn(self.pw1(out))
            out = self.act(out)
            out = self.drop(out)
            residual_branch = self.pw2_gn(self.pw2(out))
        else:
            out = self.relu(self.bn_nxt(self.conv_nxt(self.bn1(self.conv1(x)))))
            out = self.drop(out)
            residual_branch = self.bn2(self.conv2(out))
...
class ResNetEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.relu  = nn.ReLU(inplace=True) if not no_relu else nn.Identity()
        self.resnet_bottleneck = resnet_bottleneck
        self.convnext_style = convnext_style
        self.resnext_style = resnext_style
        self.layer1 = self._make_layer(block, base_channels,     layers[0], stride=1, dropout_prob=dropout_prob, no_relu=no_relu)
        self.layer2 = self._make_layer(block, base_channels * channel_mult[0], layers[1], stride=2, dropout_prob=dropout_prob, no_relu=no_relu)
        self.layer3 = self._make_layer(block, base_channels * channel_mult[1], layers[2], stride=2, dropout_prob=dropout_prob, no_relu=no_relu)
        self.layer4 = self._make_layer(block, base_channels * channel_mult[2], layers[3], stride=2, dropout_prob=dropout_prob, no_relu=no_relu)

        self.out_channels = {
            "conv1": base_channels,
            "layer1": base_channels,
            "layer2": base_channels * channel_mult[0],
            "layer3": base_channels * channel_mult[1],
            "layer4": base_channels * channel_mult[2],
        }
    ...
    def _make_layer(self, block, out_channels, blocks, stride=1, dropout_prob=0.0, no_relu=False):
        downsample = None
        if stride != 1 or self.inplanes != out_channels:
            downsample = nn.Sequential(
                nn.Conv2d(self.inplanes, out_channels, kernel_size=1, stride=stride, bias=False),
                nn.GroupNorm(_choose_gn_groups(out_channels, self.gn_max_groups), out_channels),
            )
        layers = [block(self.inplanes, out_channels, stride=stride, downsample=downsample, dropout_prob=dropout_prob, gn_max_groups=self.gn_max_groups, convnext_style=self.convnext_style, resnext_style=self.resnext_style, no_relu=no_relu, resnet_bottleneck=self.resnet_bottleneck)]
        self.inplanes = out_channels
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, out_channels, dropout_prob=dropout_prob, gn_max_groups=self.gn_max_groups, convnext_style=self.convnext_style, resnext_style=self.resnext_style, no_relu=no_relu, resnet_bottleneck=self.resnet_bottleneck))
...
class UpBlock(nn.Module):
    ...
    """
    def __init__(self, in_ch, skip_ch, out_ch, gn_max_groups: int = 32, kernel_size=3):
        super().__init__()
        self.x_norm = nn.GroupNorm(_choose_gn_groups(in_ch + skip_ch, gn_max_groups), in_ch + skip_ch)
        self.proj  = ConvBNReLU(in_ch + skip_ch, out_ch, k=kernel_size, s=1, p=kernel_size//2, gn_max_groups=gn_max_groups)
        self.refine = ConvBNReLU(out_ch, out_ch, k=kernel_size, s=1, p=kernel_size//2, gn_max_groups=gn_max_groups)
...
class UNetDecoder(nn.Module):
    ...
    def __init__(
        ...
        # Bridge 始终保留
        self.bridge = ConvBNReLU(c5, c5, k=kernel_size, s=1, p=kernel_size//2, gn_max_groups=gn_max_groups)

        if self.skip_ev_k == 0:
            # 传统路径
            self.mode = "classic"
            self.up43 = UpBlock(c5, c4, c4, gn_max_groups=gn_max_groups, kernel_size=kernel_size)   # -> layer3 scale
            self.up32 = UpBlock(c4, c3, c3, gn_max_groups=gn_max_groups, kernel_size=kernel_size)   # -> layer2 scale
            self.up21 = UpBlock(c3, c2, c2, gn_max_groups=gn_max_groups, kernel_size=kernel_size)   # -> layer1 scale
            self.up10 = UpBlock(c2, c1, c1, gn_max_groups=gn_max_groups, kernel_size=kernel_size)   # -> conv1 scale
        else:
    ...
    def forward(self, feats: Dict[str, torch.Tensor], skip_list: Optional[List[torch.Tensor]] = None) -> torch.Tensor:
        ...
        mult_by = 1 if self.use_skip else 0

        if self.skip_ev_k == 0:
            # 经典 4 步解码
            x  = self.up43(x, feats["layer3"] * mult_by)
            x  = self.up32(x, feats["layer2"] * mult_by)
            x  = self.up21(x, feats["layer1"] * mult_by)
            x  = self.up10(x, feats["conv1"] * mult_by)
            out = self.head(x)
            # 与 conv1 对齐（以防奇偶尺寸）
...
class MAEResNetGN(nn.Module):
    ...
    def __init__(
        ...
        no_encoder_relu: bool = False,
        resnet_bottleneck:int=0,
        global_loss=False,
        use_dae: bool = False, # when enabled: use flow matching as objective
    ):
        super().__init__()
        ...
        self.encoder = ResNetEncoder(
            in_channels=in_channels + (1 if concat_mask else 0),
            ...
            resnext_style=resnext_style,
            no_relu=no_encoder_relu,
    ...
    def prepare_input(self, mask, x):
        '''
        Args:
            mask: [B,1,H,W] binary mask (1 = masked, 0 = visible)
            x: [B,C,H,W] input image/latent
        Returns:
            x_masked: [B,C,H,W] or [B,C+1,H,W] if concat_mask is True
        '''
        x_masked = x * (1.0 - mask)
        if self.concat_mask:
    ...
    def forward(
        ...
        if self.use_dae:
            t_schedule = torch.rand_like(x[:, :1, :1, :1]) * (mask_ratio_max - mask_ratio_min) + mask_ratio_min
            eps = torch.randn_like(x)
            x_masked = x * (1.0 - t_schedule) + eps * (t_schedule)
            target = x - eps
            mask = torch.ones_like(x)
        else:
            mask = self._prepare_mask(x, mask, mask_ratio_min, mask_ratio_max)
            x_masked = self.prepare_input(mask, x)
            target = x
        ...
        masked_mse = (mse * loss_mask).sum() / (loss_mask.sum() + 1e-8)
        zero_recon_loss = (x ** 2 * loss_mask).sum() / (loss_mask.sum() + 1e-8)
    ...
    def activations(
        ...
        split_feats=1, # default: do nothing; when > 1: will split into that many
        concat_res: bool = False,
        concat_kth: bool = False # concat the every-k block
        ...
        mask = self._prepare_mask(x, None, 0.0, 0.0)
        x_masked = self.prepare_input(mask, x)
        feats, residuals, block_outputs = self.encoder(x_masked, return_residuals=True, return_block_outputs=True)
        res: Dict[str, torch.Tensor] = {}

        def push_tensor_once(name: str, feat: torch.Tensor, residuals: List[torch.Tensor], layer_idx: int) -> bool:
            nonlocal res
            '''
            feat: [B, C, H, W]
            '''
        ...
        def push_tensor(name: str, feat: torch.Tensor, residuals: List[torch.Tensor], layer_idx: int) -> bool:
            if split_feats == 1:
                push_tensor_once(name, feat, residuals, layer_idx)
            else:
                chunk_size = feat.shape[1] // split_feats
                for i in range(split_feats):
                    feat_split = feat[:, i * chunk_size:(i + 1) * chunk_size, :, :]
                    push_tensor_once(f"{name}_{i}", feat_split, residuals, layer_idx)
        ...
        push_tensor("layer2", feats["layer2"], residuals["layer2"], 2) # 16
        push_tensor("layer3", feats["layer3"], residuals["layer3"], 3) # 8
        push_tensor("layer4", feats["layer4"], residuals["layer4"], 4) # 4
            ...
            for lname, lidx in [("layer1", 1), ("layer2", 2), ("layer3", 3), ("layer4", 4)]:
                ...
                for i, feat_i in enumerate(blocks, start=1):
                    if i % k == 0:
                        ...
                        concats.append(feat_i)
                if len(concats) > 0:
                    concats = torch.cat(concats, dim=1)
                if concat_kth and len(concats) > 0:
                    push_tensor(f"{lname}_concat_{k}", concats, [], lidx)
        if concat_res:
            base_shape = feats['layer2'].shape
            concat_feats = []
            for s in [feats["layer2"], feats['layer3'], feats['layer4']]:
                s_interp = F.interpolate(s, size=base_shape[-2:], mode='bilinear', align_corners=False)
                concat_feats.append(s_interp)
            concat_feats = torch.cat(concat_feats, dim=1)
            push_tensor("layer_concat", concat_feats, [], 5)
...
if __name__ == "__main__":
    ...
    for convnext_style in [False, True]:
        ...
        # s = model0.activations(torch.randn(2, 4, 32, 32), concat_res=True)
        # print_module_summary(model0, [torch.randn(2, 4, 32, 32), torch.randint(0, 10, (2,))], kwargs=dict(lambda_cls=0.5, mask_ratio_min=0.5, mask_ratio_max=0.5))
